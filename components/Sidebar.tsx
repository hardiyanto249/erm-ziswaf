...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView }) => {
  ...
  return (
    <aside className="w-16 sm:w-20 md:w-64 bg-base-100 text-base-content flex flex-col border-r border-base-300">
      <div className="flex items-center justify-center h-16 sm:h-20 border-b border-base-300">
        <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-white hidden md:block">ERM Syariah</h1>
      ...
      <nav className="flex-1 px-1 sm:px-2 md:px-4 py-2 sm:py-4 space-y-1 sm:space-y-2">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => setActiveView(item.id as 'dashboard' | 'risks' | 'compliance' | 'zis-tracking')}
            className={`flex items-center w-full p-2 sm:p-3 rounded-lg transition-colors duration-200 ${
              activeView === item.id
          ...
          >
            ...
            <span className="ml-2 sm:ml-4 font-semibold hidden md:block text-sm lg:text-base">{item.label}</span>
          </button>
        ))}
      </nav>
